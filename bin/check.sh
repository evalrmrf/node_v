...
if [[ "$system_node" -ge "$engine_node" || "$system_node" -le $engine_node_second  ]] && [[ "$system_npm" -ge "$engine_npm" || "$system_npm" -le $engine_npm_second ]]
  then
    echo NodeJS v$system_node and NPM v$system_npm are up to date with package engines
    exit 0
  else
    echo system NodeJS version: $system_node and NPM version: $system_npm
    echo are not compatible with
    echo NodeJS and NPM versions defined in package.json
    echo  
fi
else
